...
def train_one_epoch_offline(model: torch.nn.Module, criterion: torch.nn.Module,
                            data_loader: Iterable, optimizer: torch.optim.Optimizer,
                            device: torch.device, epoch: int, loss_scaler, max_norm: float = 0,
                            mixup_fn: Optional[Mixup] = None, log_writer=None,
                            args=None):
    ...
    for data_iter_step, (samples, targets, mask_idxs) in enumerate(
        ...
        with torch.cuda.amp.autocast():
            outputs = model(samples, mask_idxs)
            loss = criterion(outputs, targets)
...
@torch.no_grad()
def evaluate_offline(data_loader, model, device):
    criterion = torch.nn.CrossEntropyLoss()
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        ...
        mask_idx = mask_idx.squeeze(1)
        # compute output
        with torch.cuda.amp.autocast():
            output = model(images, mask_idx=mask_idx)
            loss = criterion(output, target)
...
def train_one_epoch_multi_label_coco(model: torch.nn.Module, criterion: torch.nn.Module,
                                     data_loader: Iterable, optimizer: torch.optim.Optimizer,
                                     device: torch.device, epoch: int, loss_scaler, max_norm: float = 0,
                                     mixup_fn: Optional[Mixup] = None, log_writer=None,
                                     args=None):
    ...
    for data_iter_step, (samples, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.cuda.amp.autocast():
            outputs = model(samples, only_feature=True)
            loss = criterion(outputs, targets)
