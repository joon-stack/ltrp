...
class VWorldModelDrop(VWorldModel):
    ...
    def encode(self, obs, act, masking_ratio=0.0):
        ...
        # for k in z_dct.keys():
            ...
            # print("INFO: ids_restore: ", ids_restore)
            # print("INFO: ids_keep: ", ids_keep)
            ...
            # if self.use_patch_info or self.use_coord:
    ...
    def encodeEx(self, obs, act):
        ...
        # for k in z_dct.keys():
        #     if k == 'visual':
        #         z_dct[k] = z_dct[k][~mask].reshape(N, T, -1, D)
        # print("INFO: z_dct['visual'].shape: ", z_dct['visual'].shape)
        act_emb = z_dct['action']
        del z_dct['action']
        assert 'action' not in z_dct, f"z_dct should not have action: {z_dct.keys()}"
        ...
        if self.concat_dim == 1:
            ...
            # print("INFO: z.shape in encodeEx: ", z.shape)
    ...
    def random_masking(self, x, mask_ratio):
        ...
        x: [N, L, D], sequence
        """
        N, T, L, D = x['visual'].shape  # batch, length, dim
        len_keep = int(L * (1 - mask_ratio))
        x_masked = {}
        
        noise = torch.rand(N, L, device=x['visual'].device)  # noise in [0, 1]
        ...
        # print("INFO: N, T, L, D: ", N, T, L, D)
        # print("INFO: ids_shuffle.shape: ", ids_shuffle.shape)
        # 1. ids_shuffle을 [N, 1, L] → [N, T, L]
        ids_shuffle_expanded = ids_shuffle.unsqueeze(1).expand(-1, T, -1)  # [N, T, L]

        # 2. 마지막 D 차원을 맞춰주기 위해 [N, T, L] → [N, T, L, 1]
        ids_shuffle_expanded = ids_shuffle_expanded.unsqueeze(-1)  # [N, T, L, 1]

        # 3. D 차원도 expand → [N, T, L, D]
        ids_shuffle_expanded = ids_shuffle_expanded.expand(-1, -1, -1, D)
        # print("INFO: ids_shuffle_expanded.shape: ", ids_shuffle_expanded.shape)

        
        for k in x.keys():
            if k == 'visual':
            # shuffle the sequence
                # print("x[{}] shape: {}".format(k, x[k].shape))
                # print("ids_shuffle_expanded min:", ids_shuffle_expanded.min().item(), 
                    # "max:", ids_shuffle_expanded.max().item())
                x_masked[k] = torch.gather(x[k], dim=2, index=ids_shuffle_expanded)
            else:
                x_masked[k] = x[k]

        # generate the binary mask: 0 is keep, 1 is remove
        mask = torch.ones([N, L], device=x['visual'].device)
        mask[:, :len_keep] = 0
