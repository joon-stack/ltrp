...
# x1 = torch.tensor([[0.8, 0.2, 0]])
# x2 = torch.tensor([[0.5, 0.5, 0]])
# print(y.shape)
# print(x1.shape)
# print(mAP(y.numpy(), x1.numpy()))
# print(mAP(y.numpy(), x2.numpy()))
...
class CocoDetection(datasets.coco.CocoDetection):
    def __init__(self, root, annFile, transform=None, target_transform=None, count_max=0):
        self.root = root
        ...
        n = len(self.ids)
        if count_max > 0:
            self.ids = self.filter(count_max)
            print("CoCO after reducing :", count_max, len(self.ids) / n)


    def filter(self, count_max):
        new_ids = []
        coco = self.coco
        ...
        for img_id in self.ids:
            ...
            if (output.max(dim=0)[0]).sum() > count_max:
...
class OTE_detection(data.Dataset):
    def __init__(self, root, annot_path, transform=None, only_multi=False, count_max=0):
        super().__init__()
        self.data_dir = root
        ...
        n = len(self.data)
        if count_max > 0:
            self.data = self.filter(count_max)
            print("NUS-WIDE after reducing :", count_max, len(self.data) / n)
    ...
    def __getitem__(self, index):
        ...
        return transformed_image, obj_id
    ...
    def load_annotation(annot_path, data_dir):
        ...
        with open(annot_path) as f:
            ...
            for img_info in images_info:
                rel_image_path, img_labels = img_info['file_name'], img_info['labels']
    ...
    @staticmethod
    def load_annotation_only_multilabel(annot_path, data_dir):
        out_data = []
        with open(annot_path) as f:
            annotation = json.load(f)
            classes = sorted(annotation['classes'])
        ...
        return new_data, class_to_idx


class NUS_WIDE_detection(data.Dataset):
    def __init__(self, root, annot_path, transform=None, count_max=0):
        super().__init__()
        self.data_dir = root
        self.annot = annot_path
        self.transform = transform
        ...
        print("NUS_WIDE_detection", annot_path)
        if 'tc10' in annot_path:
            self.num_train_ids = 10
        else:
            self.num_train_ids = 21
