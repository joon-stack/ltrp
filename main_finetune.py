...
from utils.datasets import build_dataset, build_dataset_offline
from utils.pos_embed import interpolate_pos_embed
from utils.misc import NativeScalerWithGradNormCount as NativeScaler
import models_ltrp_vit
from engine_finetune import train_one_epoch, evaluate, evaluate_offline, train_one_epoch_offline
...
def main(args):
    ...
    if args.use_mask_idx:
        dataset_train = build_dataset_offline(is_train=True, args=args)
        dataset_val = build_dataset_offline(is_train=False, args=args)
    else:
    ...
    if args.eval:
        test_stats, report = evaluate(data_loader_val, model, device)
        if args.output_dir and misc.is_main_process():
            with open(os.path.join(args.output_dir, "report.txt"), mode="w", encoding="utf-8") as f:
