...
class Attention(nn.Module):
    ...
    # mask_attn [L]: 1 is remove, 0 is keep
    def forward(self, x, attn_mask):
        ...
        if attn_mask is None:
            ...
            mask[attn_mask] = 1
            attn_mask = attn_mask.unsqueeze(-2).repeat(1, N, 1)
            mask[attn_mask] = 1
...
class MaskedAutoencoderViT(nn.Module):
    ...
    def forward_encoder(self, x, mask_ratio):
        ...
        return x, mask, ids_restore, x_masked

    def forward_encoderEx(self, x, mask, attn_mask):
        # embed patches
        x = self.patch_embed(x)
        N, L, D = x.shape
    ...
    def forward(self, imgs, mask_ratio=0.75):
        latent, mask, ids_restore, x_masked = self.forward_encoder(imgs, mask_ratio)
        pred = self.forward_decoder(latent, ids_restore)  # [N, L, p*p*3]
        return latent, pred, mask, ids_restore
