...
from score_net.evit_utils.evit_layers import lecun_normal_
from score_net.dge_utils.DGE_sparse_ops import batched_sparse_attention, batched_sparse_gemm
from score_net.dge_utils.DGE_layers import DynamicGrainedEncoder
...
class Attention(nn.Module):
    ...
    def forward(self, x, q, q_lengths, return_attn=False):
        # print("q_lengths ", q_lengths)
        B, N, C = x.shape
        ...
        if not self.training:
            ...
            if (q_lengths.max() - q_lengths.min()) == 0:
                ...
                x = x.transpose(0, 1).reshape(-1, C)
        #             attn_idx = attn.mean(dim=0).argmax(dim=1)
        #             print(attn_idx)
        #             attn_idx = attn.mean(dim=0).argmax(dim=2)
        #             print(attn_idx)
...
class Block(nn.Module):
    ...
    def forward(self, x, return_attention=False, return_gates=False):
        B, N, C = x.shape

        H = W = int(math.sqrt(N))
        q = self.dge.compress(x, H, W)
        y, attn = self.attn(self.norm1(x), self.norm1(q), self.dge.states["batches"], return_attn=True)
        q = q + self.drop_path(y)
        q = q + self.drop_path(self.mlp(self.norm2(q)))
        x = self.dge.decompress(q)
        gate = self.dge.states['gate']
        indices_ = gate.argmax(dim=-1)
...
class VisionTransformer(nn.Module):
    ...
    def get_last_gates(self, x):
        ...
        for i, blk in enumerate(self.blocks):
            x, gate = blk(x, return_gates=True)
            gates.append(gate)
    ...
    def get_last_selfattention(self, x):
        ...
        gates = []
        for i, blk in enumerate(self.blocks):
            x, gate = blk(x, return_attention=True)
            gates.append(gate)
        return gates
